...
async function clearSiteData() {
  try {
    ...
    // Clear storage (DON'T clear localStorage here — we need the SHA after reload)
...
async function checkForUpdates() {
  try {
    ...
    if (!storedSha) {
      ...
      console.log("Initial commit SHA saved:", latestSha);
      return;
    }

    if (storedSha !== latestSha) {
      console.log(`New commit detected: ${storedSha.slice(0,7)} → ${latestSha.slice(0,7)}`);
      setStoredCommit(latestSha);
      await updateSite();
    } else {
      console.log("No update. Current:", latestSha.slice(0, 7));
    }
  } catch (err) {
